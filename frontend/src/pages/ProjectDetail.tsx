...
//   // Track selected reward ID from database (if exists = payment successful)
...
//   // Fetch user's selected reward - if it exists in table, payment was successful
//   useEffect(() => {
...
//         if (res.ok) {
//           const data = await res.json();
//           setSelectedRewardId(data.selected);
//           console.log("Fetched selected reward ID:", data.selected);
//         }
//       } catch (err) {
...
//     console.log("✅ Project ID ready:", project.id);
...
//             setSelectedRewardId(pendingRewardId);
//             sessionStorage.removeItem("pendingRewardId");
//             toast({ title: "Payment Successful", description: "Reward selected!" });
//             fetchProject(); // Refresh numbers
//           }
//         } catch (err) {
...
//   // Cleanup session storage on component unmount (only if navigating away from project)
//   useEffect(() => {
//     return () => {
...
//     // DESELECT: Happens only if they already PAID for this reward previously
...
//       }
//     } else {
//       // SELECT: Start process, but don't flip UI until payment success
    ...
    if (status === "success") {
      ...
      if (pendingRewardId) {
        try {
          ...
          if (response.ok) {
            // ✅ UPDATE STATE IMMEDIATELY: This flips the button to "Deselect Reward"
            setSelectedRewardId(pendingRewardId);
            sessionStorage.removeItem('pendingRewardId');
