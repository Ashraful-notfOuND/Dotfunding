...
export const getSelectedReward = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      selected: data.map(r => r.reward_id),
    });
...
export const deselectReward = async (req, res) => {
  try {
    ...
    // 3️⃣ Update counters
    const updateData = {
      backers: Math.max((reward.backers || 1) - 1, 0),
    };

    // increase available only if limited
    if (reward.available !== null) {
      updateData.available = reward.available + 1;
    }
...
export const saveRewardSelection = async (req, res) => {
  try {
    ...
    // 2️⃣ Get current reward counters
    const { data: reward, error: fetchError } = await supabase
      .from("rewards")
      .select("backers, available")
      .eq("id", rewardId)
    ...
    // 3️⃣ Update counters
    const updateData = {
      backers: (reward.backers || 0) + 1,
    };

    // only decrement available if it's a number
    if (reward.available !== null) {
      updateData.available = reward.available - 1;
    }
